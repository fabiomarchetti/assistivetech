...
import '../pages/educatore_video_page.dart';
      ...
      child: SafeArea(
        child: ListView(
          ...
          children: [
            ...
            const _AutoTimerControl(),
            const Divider(),
            // Video Educatore
            const _VideoEducatoreListTile(),
            const Divider(),
            // Condizioni d'uso
...
class _AutoTimerControl extends ConsumerWidget {
  const _AutoTimerControl();

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final isEnabled = ref.watch(autoTimerProvider);
    final interval = ref.watch(timerIntervalProvider);
          ...
          Row(
            children: [
              ...
              Switch(
                value: isEnabled,
                onChanged: (value) {
                  ref.read(autoTimerProvider.notifier).toggle();
                },
              ),
            ],
          ),
              ...
              children: [
                ...
                Expanded(
                  child: Slider(
                    ...
                    label: '${interval}s',
                    onChanged: (value) {
                      ref.read(timerIntervalProvider.notifier).setInterval(value.round());
                    },
                  ),
                ),
                const Text('60s', style: TextStyle(fontSize: 12)),
              ],
            ),
            Center(
              child: Text(
                'Avanza ogni ${interval}s',
...
class _VideoEducatoreListTile extends StatelessWidget {
  const _VideoEducatoreListTile();
  ...
  Widget build(BuildContext context) {
    return ListTile(
      ...
      subtitle: const Text('Gestisci video YouTube'),
  ...
  void _navigateToVideoPage(BuildContext context) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => const EducatoreVideoPage(),
      ),
    );
  }
}
