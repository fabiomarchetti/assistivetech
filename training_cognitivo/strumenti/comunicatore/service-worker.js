/**
 * Service Worker - Comunicatore PWA
 * Gestisce caching per funzionalità offline
 */

const CACHE_NAME = 'comunicatore-v2.5.0';
const CACHE_URLS = [
    './',
    './index.html',
    './comunicatore.html',
        ...
        caches.match(request).then((cachedResponse) => {
            ...
            return fetch(request).then((response) => {
                ...
                if (request.headers.get('accept').includes('text/html')) {
                    return caches.match('./comunicatore.html');
                }
...
self.addEventListener('push', (event) => {
    ...
    event.waitUntil(
        self.registration.showNotification('Comunicatore', options)
    );
});
...
self.addEventListener('notificationclick', (event) => {
    ...
    event.waitUntil(
        clients.openWindow('./comunicatore.html')
    );
});
